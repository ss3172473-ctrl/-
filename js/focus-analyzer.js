...
class FocusAnalyzer {
  ...
  analyzePoseLandmarks(landmarks) {
    ...
    // 디버그: 20프레임마다 값 출력 (로그 홍수 방지)
    if (this.frameData.totalFrames % 20 === 1) {
    ...
    // 고개 숙임 (코-어깨 거리가 작아짐)
    // 0.15 -> 0.12로 완화하여 민감도 낮춤
    if (noseToShoulderDist < 0.12) {
    ...
    // 옆 보기 판단 (여러 조건 OR)
    // noseOffsetX: 0.08 -> 0.1로 완화
    const isLookingAway =
      noseOffsetX > 0.1 ||       // 코가 중심에서 벗어남
      earDiff > 0.25 ||          // 귀 visibility 차이 (0.2 -> 0.25)
      eyeYDiff > 0.05;           // 눈 높이 차이 (0.03 -> 0.05)
    ...
    // 어깨 기울기 (자세 불량)
    if (shoulderTilt > 0.05) { // 0.04 -> 0.05
  ...
  calculateFocusScore() {
    ...
    const presenceScore = 100 - (this.frameData.notPresentFrames / this.frameData.totalFrames * 100);

    // 이번 턴의 원시 점수 계산 (가중 평균)
    const rawScore = (
    ...
    // Exponential Moving Average (EMA) 적용하여 점수 급변 방지
    // alpha = 0.2 (새 값 반영 비율 20%, 기존 값 유지 80%) -> 매우 부드럽게 변함
    const alpha = 0.2;
    this.focusScore = Math.round(rawScore * alpha + this.focusScore * (1 - alpha));

    // 0-100 범위로 제한
    this.focusScore = Math.max(0, Math.min(100, this.focusScore));

    console.log('[FocusAnalyzer] 점수:', this.focusScore, '(raw:', rawScore.toFixed(1), ') | head:', Math.round(headScore), '| gaze:', Math.round(gazeScore), '| presence:', Math.round(presenceScore));
